...
COOL::COOL(){
	x=15;y=5;//initial position
	movement_left = MOVE;//MOVE macro
}
void COOL::map_generate(){
#undef width
#define width 6
	srand(time(NULL));
	for(i=0;i<width;i++)
...
void COOL::candy_print(int image, int type){
	if(type > 5){
...
void COOL::eleminate_initial(){//check the map before moving
	int do_eleminate;
...
int COOL::column_eleminating(int b){
...
int COOL::row_eleminating(int a){
	bool strong_candy = FALSE;
	int row_score = 0;
	int target;
	int l;
...
int COOL::control(){
	...
	while(go_on){
		...
		c = getch();
		switch(c){
			case KEY_LEFT:  if(y!=hold_y-2 && y!=hold_y+2)x-=2;
					if(y==5 && x==13)x+=2;	
					if(y==7 && x==11)x+=2;
					break;
			case KEY_RIGHT: if(y!=hold_y-2 && y!=hold_y+2)x+=2;
					if(y==5 && x==19)x-=2;
					if(y==7 && x==21)x-=2;
					break;
			case KEY_UP:    if(x!=hold_x-2 && x!=hold_x+2)y-=2;
					if(y==5 && x==19)y+=2;
					if(y==7 && x==21)y+=2;
					if(y==5 && x==13)y+=2;	
					if(y==7 && x==11)y+=2;
					break;
			case KEY_DOWN:  if(x!=hold_x-2 && x!=hold_x+2)y+=2;
					break;
			case 'r': map_generate();
				  movement_left=MOVE;
				  score=0;
