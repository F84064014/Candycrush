#include"spec2.h"
#define BLOCK_BASE 99
//make all BLOCK different
//so they would not be able to eliminate
//and represent them with num > 99
spec2::spec2(){
	#undef width
	#define width 10
...
void spec2::map_generate(){
	srand(time(NULL));
	for(i=0;i<width;i++)
		for(j=0;j<height;j++)
			map[i][j] = rand()%5+1;//num of color
	for(i=1;i<9;i++)
		map[i][5] = BLOCK_BASE+i;
return;
}
void spec2::grid_print(){
...
void spec2::candy_print(int image, int type){
	if(type > BLOCK_BASE){
...
void spec2::eleminate_initial(){//check the map before moving
	int do_eleminate;
...
int spec2::column_eleminating(int b){
...
int spec2::row_eleminating(int a){
	bool strong_candy = FALSE;
	int row_score = 0;
	int target;
	int l;
...
void spec2::map_fall(){
	...
	for(i=0;i<width;i++){
		...
		for(j=height-1;j>=0;j--){
			if(map[i][j] > BLOCK_BASE)
				memo[j] = map[i][j];
			else if(map[i][j])
				if(memo[n] > BLOCK_BASE){
					memo[--n] = map[i][j];
					n--;
				}
				else
...
void spec2::map_refill(){
	for(i=0;i<width;i++)
		for(j=0;j<height;j++)
			if(!map[i][j])
				map[i][j] = rand()%5+1;
}
void spec2::score_print(){
	int m;
	int n=score/100;
	...
	mvprintw(13,33,"Press F to select");
	mvprintw(14,34,"Press E to quit");
}
int spec2::control(){
	static bool Hold;
