...
void spec3::map_generate(){
	srand(time(NULL));
	for(i=0;i<width;i++)
		for(j=0;j<height;j++)
			map[i][j] = rand()%5+1;//num of color
	for(i=0;i<width;i++){
		map[i][4] = BLOCK_BASE+i;
...
void spec3::candy_print(int image, int type){
	if(type >= BLOCK_BASE)
...
void spec3::eleminate_initial(){//check the map before moving
	int do_eleminate;
...
int spec3::column_eleminating(int b){
...
int spec3::row_eleminating(int a){
	bool strong_candy = FALSE;
	int row_score = 0;
	int target;
	int l;
...
void spec3::score_print(){
	...
	for(m=2*height+3;m>(21-n);m--)
		mvaddstr(m,7," ");
	attrset(A_NORMAL);
	mvprintw(3,6,"Score:%3d/%d",score,SCORE_WIN);
	mvprintw(2,5,"Move left:%2d", movement_left);
	mvprintw(13,36,"Press F to select");
	mvprintw(14,37,"Press E to quit");
}
int spec3::control(){
	static bool Hold;
